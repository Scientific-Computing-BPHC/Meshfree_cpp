...
__device__ inline void update_qtildes(double qtilde[4], double* q, double* dq1, double* dq2, double delta_x, double delta_y, int index);
__device__ inline void update_qtildes(double qtilde[4], double* q, double* dq1, double* dq2, double delta_x, double delta_y, double phi[4], int index);

__device__ void venkat_limiter(double qtilde[4], double vl_const, Point* globaldata, int index, double gamma, double phi[4], double* q, double* max_q, double* min_q)
{
...
__device__ void VLBroadcaster(double* q, double qtilde[4], double* max_q, double* min_q, double phi[4], double epsi, double del_pos, double del_neg, int index)
{
	for(int i=0; i<4; i++)
	{
		del_neg = qtilde[i + 4*threadIdx.x] - q[index*4 + i];
		if(abs(del_neg) <= 1e-5)
			phi[i] = 1.0;
...
__device__ void connectivity_stats(double x_i, double y_i, double nx, double ny, double power, double conn_x, double conn_y, double& sig_del_x_sqr, double& sig_del_y_sqr, double& sig_del_x_del_y, double& delta_x, double& delta_y, double& delta_s_weights, double& delta_n_weights)
{
 ...
 __device__ void calculate_qtile(double qtilde_i[4], double qtilde_k[4], Point* globaldata, int idx, int conn, double delta_x, double delta_y, double vl_const, \
	double gamma, int limiter_flag, double phi_i[4], double phi_k[4], \
...
__device__ inline void update_qtildes(double qtilde[4], double* q, double* dq1, double* dq2, double delta_x, double delta_y, int index)
{
	...
	{
		qtilde[iter + 4*threadIdx.x] = q[index*4 + iter] - 0.5 * (delta_x * dq1[index*4 + iter] + delta_y * dq2[index*4 + iter]);
	}
}
...
__device__ inline void update_qtildes(double qtilde[4], double* q, double* dq1, double* dq2, double delta_x, double delta_y, double phi[4], int index)
{
	for(int iter=0; iter<4; iter++)
	{
		qtilde[iter + 4*threadIdx.x] = q[index*4 + iter] - 0.5 * phi[iter] * (delta_x * dq1[index*4 + iter] + delta_y * dq2[index*4 + iter]);
	}
}
